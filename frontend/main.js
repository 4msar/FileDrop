...
const cancelChangeIdBtn = document.getElementById("cancel-change-id-btn");
...
function addPeerNode(peerId) {
  ...
  setStatus(
    `${peerNodes.size} peer${peerNodes.size > 1 ? "s" : ""} nearby — click to send`,
  );
}
...
// ── Change ID modal ──
function openChangeIdModal() {
  newIdInput.value = myPeerIdEl.textContent;
...
function isValidSlug(id) {
  // Only lowercase letters, numbers, hyphens; can't be empty
  return /^[a-z0-9-]+$/.test(id) && id.length > 0;
}

myPeerIdEl.addEventListener("click", openChangeIdModal);

cancelChangeIdBtn.addEventListener("click", closeChangeIdModal);

changeIdModal.addEventListener("click", (e) => {
  if (e.target === changeIdModal) closeChangeIdModal();
});
